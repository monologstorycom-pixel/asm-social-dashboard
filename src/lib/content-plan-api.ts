...
export function contentPlanJson(item: PlanRecord) {
  ...
  return {
    ...
    auto_approval_status: item.autoApprovalStatus,
    schedule_reason: item.scheduleReason,
    schedule_data_mode: item.scheduleDataMode,
    schedule_confidence: item.scheduleConfidence,
    schedule_sample_count: item.scheduleSampleCount,
    publisher_lease_until: item.publisherLeaseUntil instanceof Date ? item.publisherLeaseUntil.toISOString() : item.publisherLeaseUntil,
    assets,
