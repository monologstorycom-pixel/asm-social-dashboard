...
export default function OverviewClient() {
  ...
  return <div className="page-wrap">
    ...
    {error ? <StateBox title="Gagal memuat ringkasan" detail={error} action={<button onClick={() => setFilters({ ...filters })}>Coba lagi</button>} /> : <>
      <section className="kpi-grid" aria-label="Indikator kinerja utama">{kpis.map(([label, value, isPercent]) => <article className="kpi" key={label}><span>{label}</span><strong>{loading && !data ? "—" : isPercent ? percent(Number(value ?? 0)) : compactNumber.format(Number(value ?? 0))}</strong></article>)}</section>
