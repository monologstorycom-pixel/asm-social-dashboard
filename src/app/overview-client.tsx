...
type Overview = {
  ...
  accountName: string | null; username: string | null; followersCount: number | null; mediaCount: number | null; profilePictureUrl: string | null;
...
export default function OverviewClient() {
  ...
  return <div className="page-wrap">
    ...
    {data?.accountName ? <section className="state-box" style={{ marginTop: 16 }}>
      <strong>{data.accountName}</strong>
      <p>@{data.username} · {compactNumber.format(Number(data.followersCount ?? 0))} pengikut · {compactNumber.format(Number(data.mediaCount ?? 0))} media</p>
    </section> : null}
    ...
    {error ? <StateBox title="Gagal memuat ringkasan" detail={error} action={<button onClick={() => setFilters({ ...filters })}>Coba lagi</button>} /> : <>
      ...
      <section>
        ...
        {loading && !data ? <ChartSkeleton /> : data?.performanceOverTime.length ? <div className="chart"><ResponsiveContainer width="100%" height="100%"><AreaChart data={data.performanceOverTime} margin={{ top: 10, right: 8, left: -18, bottom: 0 }}><defs><linearGradient id="steelArea" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#2d7a7a" stopOpacity={0.15}/><stop offset="100%" stopColor="#2d7a7a" stopOpacity={0}/></linearGradient></defs><CartesianGrid stroke="#e5e4e1" vertical={false}/><XAxis dataKey="date" tickFormatter={(value) => dateLabel(value)} tick={{ fill: "#7a7d84", fontSize: 10 }} axisLine={false} tickLine={false} minTickGap={28}/><YAxis tickFormatter={(value) => compactNumber.format(value)} tick={{ fill: "#7a7d84", fontSize: 10 }} axisLine={false} tickLine={false}/><Tooltip labelFormatter={(value) => dateLabel(String(value))} formatter={(value) => [compactNumber.format(Number(value)), friendlyLabel(metric)]} contentStyle={{ background: "#ffffff", border: "1px solid #e5e4e1", borderRadius: 8, boxShadow: "0 1px 3px rgba(0,0,0,.06), 0 4px 12px rgba(0,0,0,.06)", fontFamily: "var(--font-body)" }}/><Area type="monotone" dataKey={metric} stroke="#2d7a7a" strokeWidth={2} fill="url(#steelArea)" activeDot={{ r: 4, fill: "#2d7a7a" }}/></AreaChart></ResponsiveContainer></div> : <EmptyInline text="Tidak ada data performa yang cocok dengan filter ini." />}
      </section>
