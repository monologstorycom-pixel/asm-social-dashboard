...
import { approvedAssetIdentity } from "@/lib/publisher-recovery";
...
export async function POST(request: Request, context: Context) {
  return safeRoute(async () => {
    ...
    const item = await db.$transaction(async (tx) => {
      ...
      const identities = new Set(current.assets.map((asset) => `${asset.revision}:${asset.candidate}`));
      if (identities.size !== 1 || current.assets.some((asset) => asset.revision !== current.assetRevision)) throw new HttpError(409, "Approval requires one explicit artifact revision and candidate");
      const candidate = current.assets[0].candidate;
      const approvedAssetSetHash = approvedAssetIdentity(current.contentId, current.assetRevision, candidate, current.assets.map((asset) => ({ slideNumber: asset.slideNumber, publicUrl: asset.publicUrl ?? "", mimeType: asset.mimeType, sha256: asset.sha256 })));
      const updated = await tx.contentPlanItem.updateMany({
        where: { id: current.id, status: "ready_for_review", approvalVersion: current.approvalVersion, assetRevision: current.assetRevision },
        data: { status: "approved", approvedAt: new Date(), approvalCommand: APPROVAL_COMMAND, approvalReference: body.reference, approvalAttemptId: randomUUID(), approvalVersion: { increment: 1 }, approvalStatus: "approved", publisherState: "ready", publisherError: null, approvedAssetSetHash, approvedCandidate: candidate },
      });
