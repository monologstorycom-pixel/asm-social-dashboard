...
export async function GET(request: Request) {
  return safeRoute(async () => {
    ...
    const date = isoDateSchema.parse(queryObject(request).date ?? currentWibDate());
    const items = await db.contentPlanItem.findMany({ where: { date: new Date(`${date}T00:00:00.000Z`) }, orderBy: { contentId: "asc" } });
