...
import { buildAgentDueWhere, claimAgentDueItem } from "@/lib/agent-due";
...
export async function GET(request: Request) {
  return safeRoute(async () => {
    ...
    const items = await db.contentPlanItem.findMany({ where: buildAgentDueWhere(), orderBy: [{ date: "asc" }, { createdAt: "asc" }], take: 100 });
...
export async function POST(request: Request) {
  return safeRoute(async () => {
    ...
    const item = await claimAgentDueItem(db, Content_ID);
    if (!item) throw new HttpError(409, "Item is not available to claim");
